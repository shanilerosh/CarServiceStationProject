...
public class CustomerController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        colStatus.setCellValueFactory(new PropertyValueFactory<>("activeInactive"));
        colCnumber.setCellValueFactory(new PropertyValueFactory<>("mobileNumber"));
        try {
            loadCustomers();
    ...
    public Customer getCustomerFromName(String name) throws SQLException, ClassNotFoundException {
        ...
        ResultSet rs = ps.executeQuery();
        while (rs.next()) {
            customer = new Customer(rs.getString(1), rs.getString(3), rs.getString(2), rs.getString(4), rs.getInt(5) == 1 ? "Active" : "Inactive");
        }
        return customer;
    }
    ...
    public void loadCustomers() throws ClassNotFoundException, SQLException {
        try {
            ...
            tblCustomerData.getColumns().addAll(colCid, colName,
                    colAdress, colCnumber, colStatus);
        } catch (ClassNotFoundException | SQLException ex) {
    ...
    public boolean addCustomer(Customer customer) throws ClassNotFoundException,
        ...
        ps.setInt(5, Integer.parseInt(customer.getActiveInactive()));
    ...
    private void btnCreate_OnAction(ActionEvent event) {
        if (txtName.getText().isEmpty()) {
            ...
            Customer newCustomer = new Customer(cid, name, address, mobile, "1");
            try {
    ...
    private ObservableList<Customer> loadCustomersByFields(String value) throws
        ...
        String sql = "SELECT * FROM Customer WHERE name LIKE ? || adress"
                + " LIKE ? || mobileNumber LIKE ? || status LIKE ?";
        PreparedStatement ps = con.prepareStatement(sql);
    ...
    private void btnActiveInactive_OnAction(ActionEvent event) throws SQLException, ClassNotFoundException {
        String status = tblCustomerData.getSelectionModel().getSelectedItem().getActiveInactive();
